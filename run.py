...
def run_action():
    ...
    wet_audio = reverb(dry_audio, sample_rate, int(entry4.get()), float(entry5.get()), int(entry6.get()), float(entry7.get()), float(entry8.get()))
    write(output_path, wet_audio, sample_rate)
    
root = tk.Tk()
root.title("Diffusion-delay reverb")
labels = ["Input file path:","Output folder:", "Output file name:","Diffusion channels:","Diffusion delay (ms):","Duffusion feedthroughs:","Feedback delay (ms):","Feedback gain (%):"]
entries = []
default_values = ["inputs/Brut_Force_Break.wav","outputs","test output.wav","8","100","1","50","0.5"]
